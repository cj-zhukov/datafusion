...
fn stats_projection(
    ...
    if primitive_row_size_possible {
        let prim_row_size = ProbabilityDistribution::new_exact(ScalarValue::UInt64(
            Some(primitive_row_size as u64),
        ))?;
        stats.total_byte_size = ProbabilityDistribution::combine_distributions(
            &Operator::Multiply,
            &prim_row_size,
            &stats.num_rows,
        )?;
