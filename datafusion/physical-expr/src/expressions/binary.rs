...
mod tests {
    ...
    fn test_evaluate_statistics_combination_of_range_holders() -> Result<()> {
        ...
        for children in [
            vec![
                ...
                &ProbabilityDistribution::new_uniform(right_interval.clone())?,
            ],
            vec![
    ...
    fn test_evaluate_statistics_bernoulli() -> Result<()> {
        ...
        assert_eq!(
            ...
            ProbabilityDistribution::new_bernoulli(ScalarValue::from(1.0 / 16.0))?
        );

        // The probability of being distinct is 1 - 1 / 16 = 15 / 16.
        assert_eq!(
            neq.evaluate_statistics(&[left_stat, right_stat])?,
            ProbabilityDistribution::new_bernoulli(ScalarValue::from(15.0 / 16.0))?
        );
    ...
    fn test_propagate_statistics_combination_of_range_holders_comparison() -> Result<()> {
        ...
        let parent = ProbabilityDistribution::new_bernoulli(one)?;
        let children = vec![
            vec![
                ProbabilityDistribution::new_uniform(left_interval.clone())?,
                ProbabilityDistribution::new_uniform(right_interval.clone())?,
